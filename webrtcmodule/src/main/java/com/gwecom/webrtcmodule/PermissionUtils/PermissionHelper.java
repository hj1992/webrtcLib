...
public class PermissionHelper {
    ...
    public boolean requestPermissions(boolean brequestPermissions) {
        checkWrapperModel(model);
      return  requestPermissionsModel(model,brequestPermissions);

    }


    private boolean requestPermissionsModel(WrapperModel pModel,boolean brequestPermissionsl) {
        boolean bRes=false;
        if (appContext == null) {
        ...
        for (String permission : pModel.permissions) {
            if (!PermissionUtil.permissionExists(appContext, permission)) {
                Log.i(TAG,permission + " must be define on the AndroidManifest.xml");
                return bRes;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        {
            ...
            if (pModel.permissions == null || pModel.permissions.length == 0) {
                ...
                if (pModel.permissionCallback instanceof PermissionCallback) {
                    //add by navy
        ...
        Log.i(TAG,"requestPermissionsModel Res:"+bRes);
        return bRes;
    }
            ...
            if (PermissionUtil.getDeniedPermissons(appContext, model.permissions).length == 0) {
                ...
                if (requestPermissionsLenth == model.permissions.length) {
                    ...
                    if (model.forceAccepting) { // 再次声请权限
                        ...
                        requestPermissionsLenth = 0;
                        requestPermissionsModel(model,true);
                    } else {
                        afterPermissonCall();
                    }
